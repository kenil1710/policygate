  ...
  async function send(functionName, args = [], value = 0n, { payees = null } = {}) {
    ...
    try {
      ...
      if (feePolicyEnabled) {
         ...
         */
        /*
         * AND an allocation per outbound external message the call might make.
         *
         * PolicyGate makes NONE: it holds no money, has no payable method and
         * never calls `emit_transfer`, so no call here can produce an external
         * message at all. The allocation is kept anyway, pointing at the sender,
         * because the deposit is a CEILING rather than a charge — whatever the
         * call does not consume comes back — and an over-allocated write costs
         * a larger number in the log, while an under-allocated one is refused
         * with `fee no_matching_allocation # external` AFTER the call has
         * already run. The cheap direction is the safe one.
         */
